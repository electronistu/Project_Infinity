...
# This module translates the rich WorldState v2 object into the token-efficient
# World-Weave Format (.wwf) v2 string.

from models import WorldState

def generate_world_weave_string(world_state: WorldState) -> str:
    """
    v2: Takes the complete WorldState object and weaves it into the final
    .wwf v2 string format for the Game Master AI.
    """
    print("[STATUS] Weaving the final v2 World-Weave Key...")
    weave_parts = []
    ...
    # CORRECTED: Removed 'ORI' from the output string.
    pc_line = (
    ...
    # --- Section: NPCs & Creatures ---
    weave_parts.append("[NPCS]")
    for npc in world_state.npcs.values():
        inv_str = ','.join([item.name for item in npc.inventory])
        npc_line = (
            ...
            f"FAC:{npc.faction_membership}|DIFF:{npc.difficulty}|GOLD:{npc.gold}|INV:{inv_str}"
        )
    ...
    if world_state.creatures:
        ...
        for cid, creature in world_state.creatures.items():
            ...
            creature_line = (
                f"{cid}|NAME:{creature.name}|T:{creature.type}|CL:{creature.challenge_level}|"
                f"LOC:{creature.location}|COORDS:{creature.coordinates[0]},{creature.coordinates[1]}|"
                f"DIFF:{creature.difficulty}|GOLD:{creature.gold}|LOOT:{loot_str}"
            )
