...
def create_gemini_chat_fn(api_key, debug=False):
        ...
        for attempt in range(max_retries):
            try:
                ...
                if status_code in (429, 500, 503) and attempt < max_retries - 1:
                    if debug:
                        console.print(f"[bold yellow]DEBUG: Gemini API error ({status_code}). Retrying... ({attempt+1}/{max_retries})[/bold yellow]")
                ...
                if attempt < max_retries - 1:
                    if debug:
                        console.print(f"[bold yellow]DEBUG: Gemini server error. Retrying... ({attempt+1}/{max_retries})[/bold yellow]")
                    await asyncio.sleep(2)
                    continue
                raise e
...
async def main():
    ...
    chat_fn = create_gemini_chat_fn(api_key, debug=debug)
